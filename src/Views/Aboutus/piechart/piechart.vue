  ...
  <div class="pie-wrapper">
    ...
    >
      ...
      <g ref="chartGroup">
        ...
        >
          <!-- glow line behind -->
          <path class="connector-line-glow" :d="slice.connectorPath" />
          <!-- main tech line (dashed, animated) -->
          <path class="connector-line-main tech-line" :d="slice.connectorPath" />
          ...
          <!-- halo behind logo -->
          <circle :cx="slice.iconX" :cy="slice.iconY" r="32" class="logo-halo" />
          <!-- logo outside pie -->
          <image
            :href="slice.logo"
        ...
        <circle class="center-circle" cx="0" cy="0" :r="innerRadius" />
        <image
          ref="centerLogoRef"
          :href="centerLogo"
          :x="-centerLogoSize / 2"
...
// Logos per slice
const rawData = [
  { label: 'ທະນາຄານແຫ່ງ ສປປ ລາວ (BOL)', value: 25, logo: '/logoallmember/circle_scale/BOL.png' },
  { label: 'ທະນາຄານ ການຄ້າຕ່າງປະເທດລາວ ມະຫາຊົນ (BCEL)', value: 20, logo: '/logoallmember/circle_scale/BCEL.png' },
  { label: 'ບໍລິສັດ ຢູນຽນເພ ສາກົນ ຈຳກັດ (UPI)', value: 15, logo: '/logoallmember/circle_scale/UPI.png' },
  { label: 'ທະນາຄານ ພັດທະນາລາວ ຈຳກັດ (LDB)', value: 10, logo: '/logoallmember/circle_scale/LDB.PNG' },
  { label: 'ທະນາຄານ ສົ່ງເສີມກະສິກຳ ຈຳກັດ (APB)', value: 10, logo: '/logoallmember/circle_scale/APBB.PNG' },
  { label: 'ທະນາຄານ ຮ່ວມທຸລະກິດລາວ-ຫວຽດ (LVB)', value: 5, logo: '/logoallmember/circle_scale/lvb.PNG' },
  { label: 'ທະນາຄານ ຮ່ວມພັດທະນາ ມະຫາຊົນ (JDB)', value: 5, logo: '/logoallmember/circle_scale/JDB.png' },
  { label: 'ທະນາຄານ ເອັສທີ ຈຳກັດ (STB)', value: 5, logo: '/logoallmember/circle_scale/STB.png' },
  { label: 'ທະນາຄານ ບີໄອຊີ ລາວ ຈຳກັດ (BIC)', value: 5, logo: '/logoallmember/circle_scale/BIC.png' }
]
...
/** ✅ shared hover state between chart + legend */
const activeIndex = ref(-1)

/** ✅ prevent flicker when moving between chart <-> legend */
const leaveTimers = new Map()

const onEnter = (index) => {
  const t = leaveTimers.get(index)
  if (t) {
    clearTimeout(t)
    leaveTimers.delete(index)
  }
...
const onLeave = (index) => {
  const prev = leaveTimers.get(index)
...
/* ✅ Technology glow at CARD BORDER (not logo) */
.legend-item.active {
...
/* optional animated gradient border using pseudo element */
.legend-item.active::before {
  ...
  animation: techGradientMove 2.8s linear infinite;
  -webkit-mask:
